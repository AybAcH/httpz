(* httplo.ml - Zero-copy HTTP/1.1 parser, standard OCaml version (no OxCaml) *)

type span = Zbufo.span = { start : Zbufo.pos; len : int }

type buffer = { zb : Zbufo.t; cur : Zbufo.cursor }
...
type header_entry = {
  name_span_start : Zbufo.pos;
  name_span_len : int;
  value_start : Zbufo.pos;
  value_len : int;
}
...
module Headers = struct
  ...
  let find_known t name : span option =
    let key = key_of_name name in
    if key < 0 then None
    else match IntMap.find_opt key t.known with
      | None -> None
      | Some (_, []) -> None
      | Some (_, e :: _) -> Some (header_entry_value e)
  ...
  let find_all_known t name : span list =
    let key = key_of_name name in
    if key < 0 then []
    else match IntMap.find_opt key t.known with
      | None -> []
      | Some (_, entries) -> List.rev_map (fun e -> header_entry_value e) entries
    ...
    | None ->
      ...
      let rec loop = function
        ...
        | unk :: rest ->
          ...
          if Zbufo.span_equal_caseless zb sp name_lower then
            Some (header_entry_value unk.unk_entry)
          else loop rest
      in
      loop t.unknown
...
let create () =
  ...
  { zb; cur = Zbufo.cursor_global zb ~pos:0 }

let push buf data ~off ~len = Zbufo.push buf.zb data ~off ~len

let with_refill buf refill f =
  Zbufo.with_refill buf.zb (fun _ -> refill buf) f

let[@inline] mark buf = Zbufo.cursor_mark buf.cur
let[@inline] peek buf off = Zbufo.peek buf.zb buf.cur off
let[@inline] try_refill buf n = Zbufo.ensure buf.zb buf.cur n
let[@inline] consume buf n = Zbufo.advance buf.cur n
...
let[@inline] span_to_string buf sp = Zbufo.span_to_string buf.zb sp
let[@inline] span_equal buf sp s = Zbufo.span_equal buf.zb sp s
let[@inline] span_equal_caseless buf sp s = Zbufo.span_equal_caseless buf.zb sp s
let[@inline] span_iter f buf sp = Zbufo.span_iter f buf.zb sp
let[@inline] span_fold f buf sp init = Zbufo.span_fold f buf.zb sp init
...
let take_while1 pred buf =
  Zbufo.cursor_mark buf.cur;
  ignore (Zbufo.scan_while pred buf.zb buf.cur);
  let sp = Zbufo.span_since_mark buf.cur in
  if sp.len = 0 then raise (Parse_error Partial);
  sp

let[@inline] skip_ows buf = ignore (Zbufo.scan_while is_space buf.zb buf.cur)
...
let parse_header buf hdrs =
  if peek buf 0 = '\r' && peek buf 1 = '\n' then begin
    ...
    consume buf 1;
    skip_ows buf;
    let value_start = Zbufo.cursor_pos buf.cur in
    let crlf_off = find_crlf buf in
    let end_off = ref crlf_off in
...
let parse_reason buf =
  let start = Zbufo.cursor_pos buf.cur in
  let crlf_off = find_crlf buf in
  consume buf (crlf_off + 2);
  (start, crlf_off)
...
let get_header (buf : buffer) (req : request) name =
  Option.map (Zbufo.span_to_string buf.zb) (Headers.find_by_string buf.zb req.headers name)

let get_header_by_name (buf : buffer) (req : request) target =
  Option.map (Zbufo.span_to_string buf.zb) (Headers.find_known req.headers target)

let get_headers_by_name (buf : buffer) (req : request) target =
  List.map (Zbufo.span_to_string buf.zb) (Headers.find_all_known req.headers target)

let content_length (buf : buffer) (req : request) =
  Option.bind (Headers.find_known req.headers H_content_length)
    (fun sp -> Int64.of_string_opt (Zbufo.span_to_string buf.zb sp))

let[@inline] is_chunked (buf : buffer) (req : request) =
  Headers.with_known req.headers H_transfer_encoding ~default:false
    ~f:(fun sp -> Zbufo.span_equal_caseless buf.zb sp "chunked")

let[@inline] is_keep_alive (buf : buffer) (req : request) =
  Headers.with_known req.headers H_connection ~default:(req.version = HTTP_1_1)
    ~f:(fun sp ->
      if Zbufo.span_equal_caseless buf.zb sp "close" then false
      else if Zbufo.span_equal_caseless buf.zb sp "keep-alive" then true
...
let get_header_resp (buf : buffer) (resp : response) name =
  Option.map (Zbufo.span_to_string buf.zb) (Headers.find_by_string buf.zb resp.headers name)
...
let write_out dst ~pos = function
  ...
  | Span (buf, sp) -> Zbufo.span_blit buf.zb sp dst ~dst_off:pos; sp.len
...
let write_method_from ~src dst ~pos = function
  | Other sp -> Zbufo.span_blit src.zb sp dst ~dst_off:pos; sp.len
...
let rec body_read buf reader dst ~off ~len =
  ...
  | Fixed_body state ->
    if state.remaining = 0L then 0
      ...
      let available = Zbufo.cursor_remaining buf.zb buf.cur in
      if available = 0 then
        if try_refill buf 1 then body_read buf reader dst ~off ~len else 0
      else begin
        let to_read = min want available in
        let sp = { start = Zbufo.cursor_pos buf.cur; len = to_read } in
        Zbufo.span_blit buf.zb sp dst ~dst_off:off;
        consume buf to_read;
...
let body_read_span buf reader : span option =
  match reader with
  | Empty_body -> Some { start = Zbufo.cursor_pos buf.cur; len = 0 }
  | Fixed_body state ->
    let len = Int64.to_int state.remaining in
    if len > 1024 * 1024 || not (try_refill buf len) then None
    else begin
      let sp = { start = Zbufo.cursor_pos buf.cur; len } in
      consume buf len;
      state.remaining <- 0L;
      Some sp
    end
...
let body_read_string buf reader =
  ...
  | Fixed_body state when state.remaining <= 65536L ->
    (match body_read_span buf reader with
     | Some sp -> Zbufo.span_to_string buf.zb sp
     | None -> "")
  | _ ->
...
let rec body_stream buf reader callback =
  ...
  | Fixed_body state ->
    if state.remaining = 0L then
      ...
      if available = 0 then begin
        ...
        let sp = { start = Zbufo.cursor_pos buf.cur; len = want } in
        consume buf want;
  ...
  | Close_body ->
    ...
    if available = 0 then begin
      ...
      let sp = { start = Zbufo.cursor_pos buf.cur; len = available } in
