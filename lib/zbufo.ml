(* zbufo.ml - Zero-copy multi-buffer spans, standard OCaml version (no OxCaml) *)
...
let push t data ~off ~len =
  if off < 0 || len < 0 || off + len > Bytes.length data then
    invalid_arg "Zbufo.push: invalid offset or length";
...
let cursor t ~pos =
  if pos < 0 || pos > t.total_len then
    invalid_arg "Zbufo.cursor: position out of bounds";
  { pos; mark = pos; frag_idx = find_frag_idx_from t pos 0 }

let cursor_global t ~pos =
  if pos < 0 || pos > t.total_len then
    invalid_arg "Zbufo.cursor_global: position out of bounds";
  { pos; mark = pos; frag_idx = find_frag_idx_from t pos 0 }
...
let cursor_set t cur new_pos =
  if new_pos < 0 || new_pos > t.total_len then
    invalid_arg "Zbufo.cursor_set: position out of bounds";
  cur.pos <- new_pos;
  let count = Dynarray.length t.fragments in
...
let rec peek t cur off =
  ...
  if pos < 0 then invalid_arg "Zbufo.peek: negative position";
  if pos >= t.total_len then begin
    ...
    if cur.frag_idx < count then begin
      ...
      if pos >= frag.base && pos < frag.base + frag.len then
        ...
        let idx = ref (cur.frag_idx + 1) in
        let result = ref '\x00' in
        let found = ref false in
        while not !found do
    ...
    end else invalid_arg "Zbufo.peek: inconsistent state"
  end

let[@inline] advance cur n = cur.pos <- cur.pos + n
...
let get t pos =
  if pos < 0 || pos >= t.total_len then invalid_arg "Zbufo.get: position out of bounds";
  let idx = ref 0 in
  let result = ref '\x00' in
  let found = ref false in
  while not !found do
...
let span_to_string t (sp : span) =
  if sp.len = 0 then ""
    ...
    if sp.start < 0 || sp.start + sp.len > t.total_len then
      invalid_arg "Zbufo.span_to_string: span out of bounds";
    let result = Bytes.create sp.len in
    let start_idx = find_frag_idx t sp.start in
    let dst_off = ref 0 in
    let idx = ref start_idx in
    let pos = ref sp.start in
    let remaining = ref sp.len in
...
let span_equal t sp s =
  span_equal_with ~char_equal:Char.equal ~err_msg:"Zbufo.span_equal: span out of bounds" t sp s

let span_equal_caseless t sp s =
  span_equal_with
    ~char_equal:(fun c1 c2 -> Char.lowercase_ascii c1 = Char.lowercase_ascii c2)
    ~err_msg:"Zbufo.span_equal_caseless: span out of bounds" t sp s

let span_iter f t (sp : span) =
  if sp.len > 0 then begin
    if sp.start < 0 || sp.start + sp.len > t.total_len then
      invalid_arg "Zbufo.span_iter: span out of bounds";
    let start_idx = find_frag_idx t sp.start in
    let idx = ref start_idx in
    let pos = ref sp.start in
    let remaining = ref sp.len in
...
let span_fold f t (sp : span) init =
  if sp.len = 0 then init
    ...
    if sp.start < 0 || sp.start + sp.len > t.total_len then
      invalid_arg "Zbufo.span_fold: span out of bounds";
    let start_idx = find_frag_idx t sp.start in
    let acc = ref init in
    let idx = ref start_idx in
    let pos = ref sp.start in
...
let span_blit t (sp : span) dst ~dst_off =
  if sp.len > 0 then begin
    if sp.start < 0 || sp.start + sp.len > t.total_len then
      invalid_arg "Zbufo.span_blit: span out of bounds";
    if dst_off < 0 || dst_off + sp.len > Bytes.length dst then
      invalid_arg "Zbufo.span_blit: destination out of bounds";
    let start_idx = find_frag_idx t sp.start in
