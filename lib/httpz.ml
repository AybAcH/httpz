...
let create_buffer () = Base_bigstring.create buffer_size
let[@inline always] peek buf pos = Base_bigstring.unsafe_get buf pos
...
let[@inline always] to_lower c =
  ...
  then Char.unsafe_of_int (Char.to_int c + 32)
  else c
;;
...
let[@inline] span_equal_caseless buf (sp : span) s =
  ...
  then false
    ...
    while equal && i < slen do
      if to_lower (peek buf (off + i)) <>. to_lower (String.unsafe_get s i)
      then equal <- false
      else i <- i + 1
    done;
    equal)
...
let parse_header_name buf (sp : span) : header_name =
  match sp.#len with
  | 3 ->
    if span_equal_caseless buf sp "age"
    then H_age
    else if span_equal_caseless buf sp "via"
    then H_via
    else H_other
  | 4 ->
    if span_equal_caseless buf sp "date"
    then H_date
    else if span_equal_caseless buf sp "etag"
    then H_etag
    else if span_equal_caseless buf sp "host"
    then H_host
    else H_other
  | 5 ->
    if span_equal_caseless buf sp "allow"
    then H_allow
    else if span_equal_caseless buf sp "range"
    then H_range
    else H_other
  | 6 ->
    if span_equal_caseless buf sp "accept"
    then H_accept
    else if span_equal_caseless buf sp "cookie"
    then H_cookie
    else if span_equal_caseless buf sp "expect"
    then H_expect
    else if span_equal_caseless buf sp "server"
    then H_server
    else H_other
  | 7 ->
    if span_equal_caseless buf sp "expires"
    then H_expires
    else if span_equal_caseless buf sp "referer"
    then H_referer
    else if span_equal_caseless buf sp "upgrade"
    then H_upgrade
    else H_other
  | 8 ->
    if span_equal_caseless buf sp "if-match"
    then H_if_match
    else if span_equal_caseless buf sp "location"
    then H_location
    else H_other
  | 10 ->
    if span_equal_caseless buf sp "connection"
    then H_connection
    else if span_equal_caseless buf sp "set-cookie"
    then H_set_cookie
    else if span_equal_caseless buf sp "user-agent"
    then H_user_agent
    else H_other
  | 11 -> if span_equal_caseless buf sp "retry-after" then H_retry_after else H_other
  | 12 ->
    if span_equal_caseless buf sp "content-type"
    then H_content_type
    else if span_equal_caseless buf sp "x-request-id"
    then H_x_request_id
    else H_other
  | 13 ->
    if span_equal_caseless buf sp "authorization"
    then H_authorization
    else if span_equal_caseless buf sp "cache-control"
    then H_cache_control
    else if span_equal_caseless buf sp "content-range"
    then H_content_range
    else if span_equal_caseless buf sp "if-none-match"
    then H_if_none_match
    else if span_equal_caseless buf sp "last-modified"
    then H_last_modified
    else H_other
  | 14 ->
    if span_equal_caseless buf sp "accept-charset"
    then H_accept_charset
    else if span_equal_caseless buf sp "content-length"
    then H_content_length
    else H_other
  | 15 ->
    if span_equal_caseless buf sp "accept-encoding"
    then H_accept_encoding
    else if span_equal_caseless buf sp "accept-language"
    then H_accept_language
    else if span_equal_caseless buf sp "x-forwarded-for"
    then H_x_forwarded_for
    else if span_equal_caseless buf sp "x-correlation-id"
    then H_x_correlation_id
    else H_other
  | 16 ->
    if span_equal_caseless buf sp "content-encoding"
    then H_content_encoding
    else if span_equal_caseless buf sp "content-language"
    then H_content_language
    else if span_equal_caseless buf sp "content-location"
    then H_content_location
    else if span_equal_caseless buf sp "www-authenticate"
    then H_www_authenticate
    else if span_equal_caseless buf sp "x-forwarded-host"
    then H_x_forwarded_host
    else H_other
  | 17 ->
    if span_equal_caseless buf sp "if-modified-since"
    then H_if_modified_since
    else if span_equal_caseless buf sp "transfer-encoding"
    then H_transfer_encoding
    else if span_equal_caseless buf sp "x-forwarded-proto"
    then H_x_forwarded_proto
    else H_other
  | 19 ->
    if span_equal_caseless buf sp "content-disposition"
    then H_content_disposition
    else if span_equal_caseless buf sp "if-unmodified-since"
    then H_if_unmodified_since
    else H_other
  | _ -> H_other
;;
...
let[@inline] parse_method buf ~pos ~len : #(status * method_ * int) =
  ...
  if meth_len = 0
  then #(Invalid_method, GET, 0)
  else (
    let sp = #{ off = pos; len = meth_len } in
    let meth =
      match meth_len with
      | 3 ->
        if span_equal buf sp "GET"
        then GET
        else if span_equal buf sp "PUT"
        then PUT
        else Other sp
      | 4 ->
        if span_equal buf sp "POST"
        then POST
        else if span_equal buf sp "HEAD"
        then HEAD
        else Other sp
      | 5 ->
        if span_equal buf sp "PATCH"
        then PATCH
        else if span_equal buf sp "TRACE"
        then TRACE
        else Other sp
      | 6 -> if span_equal buf sp "DELETE" then DELETE else Other sp
      | 7 ->
        if span_equal buf sp "OPTIONS"
        then OPTIONS
        else if span_equal buf sp "CONNECT"
        then CONNECT
        else Other sp
      | _ -> Other sp
    in
...
let[@inline] parse_target buf ~pos ~len : #(status * int * int * int) =
  ...
  if target_len = 0 then #(Invalid_target, 0, 0, 0) else #(Ok, pos, target_len, p)
;;

let[@inline] parse_version buf ~pos ~len : #(status * version * int) =
  if pos + 8 > len
  then #(Partial, HTTP_1_1, 0)
  else if peek buf pos =. 'H'
          && peek buf (pos + 1) =. 'T'
          && peek buf (pos + 2) =. 'T'
          && peek buf (pos + 3) =. 'P'
          && peek buf (pos + 4) =. '/'
  ...
  then (
    ...
    if minor =. '1'
    then #(Ok, HTTP_1_1, pos + 8)
    else if minor =. '0'
    then #(Ok, HTTP_1_0, pos + 8)
  ...
  then -1
    ...
    while (not found) && p + 1 < len do
      ...
      then
        ...
        p <- len
      else if Base_bigstring.unsafe_get buf (cr_pos + 1) =. '\n'
      then (
        p <- cr_pos;
        found <- true)
...
let[@inline] parse_header buf ~pos ~len
  : #(status * header_name * int * int * int * int * int)
  =
  (* Find colon *)
  ...
  if name_len = 0 || colon_pos >= len || peek buf colon_pos <>. ':'
  then #(Invalid_header, H_host, 0, 0, 0, 0, 0)
  else (
...
let rec parse_headers_loop buf ~pos ~len ~count ~acc ~content_len ~chunked ~conn
  = exclave_
  if pos + 1 >= len
  then #(Partial, pos, 0, acc, content_len, chunked, conn)
  else if peek buf pos =. '\r' && peek buf (pos + 1) =. '\n'
  then (* End of headers *)
    #(Ok, pos + 2, count, acc, content_len, chunked, conn)
  else if count >= max_headers
  then #(Headers_too_large, pos, count, acc, content_len, chunked, conn)
  else (
    let #(s, name, noff, nlen, voff, vlen, new_pos) = parse_header buf ~pos ~len in
    if Poly.( <> ) s Ok
    then #(s, pos, count, acc, content_len, chunked, conn)
    else (
      ...
      | H_connection ->
        let new_conn =
          if span_equal_caseless buf value_span "close"
          then 1
          else if span_equal_caseless buf value_span "keep-alive"
          then 2
          else conn
        in
        parse_headers_loop
...
let parse buf ~len = exclave_
  ...
  in
  if len > buffer_size
  then error_result Headers_too_large
  else (
    (* Parse method *)
    ...
    then error_result Partial
    else if peek buf pos <>. ' '
    then error_result Invalid_method
    else (
      let pos = pos + 1 in
      (* Parse target *)
      ...
      then error_result Invalid_target
      else (
        let pos = pos + 1 in
        (* Parse version *)
        ...
        then error_result Malformed
          ...
          | Ok ->
            ...
            in
            let req =
              #{ meth; target; version; body_off; content_length; is_chunked; keep_alive }
            in
            #(Ok, req, headers)
...
let body_bytes_needed ~len (req : request @ local) =
  ...
  then -1
    ...
    then 0
      ...
      let body_len = Int64.to_int_exn cl in
      let body_end = req.#body_off + body_len in
      if body_end <= len then 0 else body_end - len))
;;
...
let parse_chunk buf ~off ~len =
  ...
  if off >= len
  then #(Chunk_partial, empty_chunk)
  else (
    (* Parse hex size *)
    ...
    then #(Chunk_error, empty_chunk)
      ...
      then #(Chunk_error, empty_chunk)
        ...
        then
          ...
          then #(Chunk_done, #{ data_off; data_len = 0; next_off = data_off + 2 })
          else #(Chunk_done, #{ data_off; data_len = 0; next_off = data_off })
        else (
          ...
          then #(Chunk_error, empty_chunk)
          else #(Chunk_ok, #{ data_off; data_len = size; next_off = data_end + 2 })))))
;;
...
let write_content_length dst ~off len = write_header_int dst ~off "Content-Length" len

let write_connection dst ~off keep_alive =
  if keep_alive
  then write_header dst ~off "Connection" "keep-alive"
...
let write_connection_bigstring dst ~off keep_alive =
  if keep_alive
  then write_header_bigstring dst ~off "Connection" "keep-alive"
